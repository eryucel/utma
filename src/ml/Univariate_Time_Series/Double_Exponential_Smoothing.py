...
class Double_Exponential_Smoothing:
  def __init__(self,path,time_freq,trend="add",seasonal=None,time_column=0,day_to_month=False,month_to_year=False,test_size=0.2,model="additive"):
  ...
  def best_alpha__beta_value(self,alphas, betas, step):
    ...
    for alpha in alphas:
        for beta in betas:
            des_model = ExponentialSmoothing(self.y_train, trend=self.trend).fit(smoothing_level=alpha, smoothing_slope=beta)
  ...
  def forecast_and_visualize(self):
    ...
    forecaster =ExponentialSmoothing(self.y_train, trend=self.trend).fit(smoothing_level=alpha,smoothing_slope=beta)
