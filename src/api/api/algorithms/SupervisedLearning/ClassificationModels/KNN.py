...
class KNearestNeighbors():
    ...
    def optimized_training(self, args):
        ...
        self.y_pred_linear = self.regr.predict(self.X_test)
        result = {'cross_track': log_loss(self.y_test, self.y_pred),
                  'accuracy': accuracy_score(self.y_test, self.y_pred_linear),
                  'best_k': args['n_neighbors']}
        return result
    ...
    def visualize(self, task_id, optimized=False):
        ...
        if optimized:
            plt.savefig("files/KNN_" + str(task_id) + "_optimized.png", dpi=100)
            return "KNN_" + str(task_id) + "_optimized.png"
        else:
            plt.savefig("files/KNN_" + str(task_id) + ".png", dpi=100)
            return "KNN_" + str(task_id) + ".png"
    ...
    def visualize_classes(self, task_id, optimized=False):
        ...
        if optimized:
            plt.savefig("files/KNN_classes_" + str(task_id) + "_optimized.png", dpi=100)
            return "KNN_classes_" + str(task_id) + "_optimized.png"
        else:
            plt.savefig("files/KNN_classes_" + str(task_id) + ".png", dpi=100)
            return "KNN_classes_" + str(task_id) + ".png"
    ...
    def run_optimized_model(self, task_id):
        ...
        return {'training': self.optimized_training(args), 'visualize': self.visualize(task_id, optimized=True),
                'visualize_classes': self.visualize_classes(task_id, optimized=True),
                'classification_report': self.classification_report(),
                'best_parameters': {'algorithm': self.best_parameters['param']['algorithm'],
                                    'leaf_size': self.best_parameters['param']['leaf_size'].astype(str),
                                    'n_neighbors': self.best_parameters['param']['n_neighbors'],
                                    'weights': self.best_parameters['param']['weights']}}
