...
class Triple_Exponential_Smoothing:
  ...
  def best_alpha__beta_value(self,abg, step):
    results = []
    for comb in abg:
        tes_model = ExponentialSmoothing(self.y_train, trend=self.trend, seasonal=self.seasonal, seasonal_periods=self.sp).fit(smoothing_level=comb[0], smoothing_slope=comb[1], smoothing_seasonal=comb[2])
  ...
  def forecast_and_visualize(self):
    ...
    forecaster = ExponentialSmoothing(self.y_train, trend=self.trend,seasonal=self.seasonal,seasonal_periods=self.sp).fit(smoothing_level=alpha,smoothing_slope=beta,smoothing_seasonal=gamma)
