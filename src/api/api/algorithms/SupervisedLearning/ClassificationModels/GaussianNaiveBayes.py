...
class GaussianNaiveBayes():
    ...
    def training(self, train_test_split=True):
        ...
        result = {'cross_track': log_loss(self.y_test, self.y_pred),
                  'accuracy': self.regr.score(self.X_test, self.y_test)}
        return result
    ...
    def visualize(self, task_id, optimized=False):
        ...
        if optimized:
            plt.savefig("files/gaussianNaiveBayes_" + str(task_id) + "_optimized.png", dpi=100)
            return "gaussianNaiveBayes_" + str(task_id) + "_optimized.png"
        else:
            plt.savefig("files/gaussianNaiveBayes_" + str(task_id) + ".png", dpi=100)
            return "gaussianNaiveBayes_" + str(task_id) + ".png"
    ...
    def visualize_classes(self, task_id, optimized=False):
        ...
        if optimized:
            plt.savefig("files/gaussianNaiveBayes_classes_" + str(task_id) + "_optimized.png", dpi=100)
            return "gaussianNaiveBayes_classes_" + str(task_id) + "_optimized.png"
        else:
            plt.savefig("files/gaussianNaiveBayes_classes_" + str(task_id) + ".png", dpi=100)
            return "gaussianNaiveBayes_classes_" + str(task_id) + ".png"
